...
from methods import spatial, shapefiles, function, geo
...
class Preprocess(PostgreSQL):
        ...
        def get_toponyms(names):
            ...
            if names_no_exist:
                ...
                for i, name in enumerate(names_no_exist, start=1):
                    ...
                    self.cur.execute("SELECT geonameid \
                        FROM geonames \
                    ...
                    self.cur.execute("SELECT geonameid, isolanguage \
                        FROM alternate_names \
                        WHERE alternate_name='{}'".format(name.replace("'", "''")))

                    for id, isolanguage in self.cur.fetchall():
                        if isolanguage is not None:
                            if id not in languages:
                                languages[id] = []
                            languages[id].append(isolanguage)
                    if languages:
                        ids = '(' + ', '.join([str(id) for id in languages.keys()]) + ')'

                        self.cur.execute(f"""SELECT geonameid,
                                name,
                                ST_X(location),
                                ST_Y(location),
                                population,
                        ...
                        locations = [
                                ...
                                for geonameid, name, longitude, latitude, population, feature_code, feature_class, country_geonameid, adm1_geonameid, time_zone in self.cur.fetchall()
            ...
            print("Indexing unique names ({}/{})".format(i, n_names))
    ...
    def create_continent_table(self):
        ...
        if not self.cur.fetchone()[0]:
            ...
            self.cur.execute("INSERT INTO continents (geonameid, geometry) VALUES " + arg_str)
            self.conn.commit()
    ...
    def create_country_table(self):
        ...
        if not self.cur.fetchone()[0]:
            ...
            self.cur.execute("INSERT INTO countries (geonameid, ISO2, ISO3, continents, languages, name, geometry) VALUES " + arg_str)
    ...
    def create_adm1_table(self):
        ...
        if not self.cur.fetchone()[0]:
            ...
            self.cur.execute("CREATE TABLE adm1 ( \
                geonameid INTEGER PRIMARY KEY, \
                geometry GEOMETRY(Geometry, {SRID}) \
            )".format(SRID=SRID))
            ...
            arg_str = ','.join(self.cur.mogrify("(%s, ST_GeometryFromText(%s, {SRID}))".format(SRID=SRID), value).decode() for value in areas.items())
            self.cur.execute("INSERT INTO adm1 (geonameid, geometry) VALUES " + arg_str)
            self.conn.commit()
    ...
    def find_administrative_parents(self):
        ...
        if not self.cur.fetchone()[0]:
            ...
            self.cur.execute("""
                ...
                FROM adm1
                WHERE (ST_Within(geonames.location, adm1.geometry) AND feature_code in ('PPL','PPLA','PPLA2','PPLA3','PPLA4','PPLC','PPLG','PPLR','PPLS','PPLX','STLMT'))
            """)
            self.conn.commit()
            self.cur.execute("""
                UPDATE geonames
    ...
    def find_time_zones(self):
        ...
        if not self.cur.fetchone()[0]:
            ...
            self.cur.execute("""
                ...
                FROM time_zones
                WHERE ST_Within(geonames.location, time_zones.geometry)
            """)
            self.conn.commit()
    ...
    def create_geonames_table(self):
        ...
        if not self.cur.fetchone()[0]:
            ...
            query = """
                INSERT INTO geonames (geonameid, name, feature_class, feature_code, country_geonameid, population, location)
                VALUES {}
            """
            mogr = f"(%s, %s, %s, %s, %s, %s, ST_GeomFromText('POINT(%s %s)', {SRID}))"
    ...
    def create_alternate_names_table(self):
        ...
        if not self.cur.fetchone()[0]:
            ...
            # Commit to database
            query = "INSERT INTO alternate_names (alternateNameId, geonameid, isolanguage, alternate_name) VALUES {}"
            mogr = "(%s, %s, %s, %s)"
    ...
    def create_time_zone_table(self):
        ...
        if not self.cur.fetchone()[0]:
            ...
            query = "INSERT INTO time_zones (name, geometry) VALUES {}"
    ...
    def get_most_common_words_web(self):
        ...
        if not self.cur.fetchone()[0]:
            ...
            for language_code, language in languages.items():
                if language_code != 'en':
                    url = f'http://1000mostcommonwords.com/1000-most-common-{language}-words/'
                else:
                    url = 'http://1000mostcommonwords.com/1000-common-english-words/'
                r = requests.get(url)
                ...
                if language_code == 'en':
                    ...
                    index = 1
                tags = (row.find_all('td')[index].text.strip() for row in rows)
                for i, word in enumerate(tags, start=1):
                    words.append((i, language_code, word))
