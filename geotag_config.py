...
# Number of processes to use for analyzing individual tweets
N_PROCESSES = 3
...
# A user home location and entity are considered a match if closer than:
USER_ENTITY_DISTANCE = 200000  # m
...
pg_spatial = PostgreSQL(POSTGRESQL_DB)
...
class GeotagCustom:
    ...
    def locations_to_commit(self, fully_resolved):
                    ...
                    new_locations.append(
                        next(
                            (db_loc for db_loc in db_locations
                                if db_loc['toponym'] == loc['toponym'] and loc['avg_score'] > db_loc['avg_score']), loc
                            )
                    )
            if db_locations != new_locations:
                tweet['locations'] = new_locations
                if UPDATE:
                    body = {
    ...
    def analyze_tweets(self, query):
        ...
        if N_PROCESSES > 1:
            p = Pool(N_PROCESSES)
            loc_tweets = p.map(self.tweet_analyzer.analyze_tweet, tweets)
            p.close()
            # Could be improved slightly by passing manager.dict to multiprocessing
            loc_tweets = {
                loc_tweet[0]: loc_tweet[1]
                for loc_tweet in loc_tweets
                if loc_tweet is not None
            }
        else:
            loc_tweets = dict([(item[0], item[1]) for item in [
                    self.tweet_analyzer.analyze_tweet(tweet)
                    for tweet in tweets] if item is not None
                ]
            )
